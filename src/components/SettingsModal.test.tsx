...
describe('SettingsModal', () => {
  beforeEach(() => {
      ...
      if (url === '/api/videos/status') {
        return Promise.resolve({
          ...
          json: async () => ({
            errors: 1,
            failedChannels: [
              { id: 'UC_BAD', title: 'Broken Channel', reason: 'No RSS videos or metadata returned' },
            ],
          }),
        });
      }
    ...
    const storage = new Map<string, string>([
      ...
      ['feed-quality-filters', JSON.stringify({ hidePremieres: true, mutedKeywordText: 'rumor' })],
    ]);
  ...
  it('resets feed cache without clearing saved user data', async () => {
    ...
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith('/api/videos/cache/reset', { method: 'POST' });
    });
  ...
  it('shows failed refresh channels in Settings', async () => {
    ...
    expect(screen.getByText('No RSS videos or metadata returned')).toBeInTheDocument();
  });
  ...
  it('can retry failed channel refreshes from Settings', async () => {
    ...
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith('/api/videos/refresh', { method: 'POST' });
    });
    expect(invalidateQueries).toHaveBeenCalledWith({ queryKey: ['server-videos'] });
    expect(invalidateQueries).toHaveBeenCalledWith({ queryKey: ['server-videos-status'] });
