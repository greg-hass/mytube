...
describe("AddChannelModal", () => {
	...
	it("shows a preview before adding a searched channel", async () => {
		...
		const { container } = render(
			<AddChannelModal isOpen onClose={onClose} onAdd={onAdd} />,
		);
		...
		fireEvent.click(
			screen.getByRole("button", { name: /preview linux tech channel/i }),
		);
		...
		expect(
			within(previewCard as HTMLElement).getByText("Linux Tech Channel"),
		).toBeInTheDocument();
	...
	it("sends natural-language search phrases to the backend unchanged", async () => {
		vi.stubGlobal(
			...
			vi.fn((url: string | URL | Request) => {
				...
				if (
					requestUrl ===
					"/api/channel-search?q=the%20best%20woodworking%20channels"
				) {
					return Promise.resolve({
						ok: true,
						json: () =>
		...
		fireEvent.click(
			screen.getByRole("button", { name: /preview workshop companion/i }),
		);

		const previewCard = screen.getByText("Channel Preview").closest("section");
		expect(previewCard).not.toBeNull();
		expect(
			within(previewCard as HTMLElement).getByText("250,000 subscribers"),
		).toBeInTheDocument();
		expect(
			within(previewCard as HTMLElement).getByText("/@workshopcompanion"),
		).toBeInTheDocument();
	});
	...
	it('surfaces an authentication-required message on 401 instead of "no channels found"', async () => {
		...
		// Error should NOT be swallowed via console.error for HTTP 401 — that was the bug.
