...
const SHORTS_TEXT_PATTERN =
	/#shorts?\b|#ytshorts?\b|#fyp\b|\bshorts\b|youtube\.com\/shorts\//i;
const LIVE_REPLAY_TEXT_PATTERN =
	/\b(live\s*stream|livestream|watchalong|replay|full\s+stream)\b/i;
...
function isSquareOrVerticalVideo(
	video: Pick<YouTubeVideo, "videoWidth" | "videoHeight">,
) {
...
export function isShortVideo(
	...
	if (!duration || duration <= 0) {
		return SHORTS_TEXT_PATTERN.test(
	...
	if (
		duration <= DIMENSION_BASED_SHORTS_MAX_SECONDS &&
		isSquareOrVerticalVideo(video)
	) {
		return true;
	}
...
export function isLiveReplayVideo(
	video: Pick<YouTubeVideo, "title" | "description">,
) {
	return LIVE_REPLAY_TEXT_PATTERN.test(
		`${video.title || ""} ${video.description || ""}`,
	);
}

export function isPremiereVideo(
	video: Pick<YouTubeVideo, "title" | "description">,
) {
	return PREMIERE_TEXT_PATTERN.test(
		`${video.title || ""} ${video.description || ""}`,
	);
}

function matchesDurationFilter(
	video: VideoWithNullableDuration,
	...
	if (durationFilter === "10-30")
		return duration >= TEN_MINUTES_SECONDS && duration < THIRTY_MINUTES_SECONDS;
...
export function buildVideoFeedIndex(
	videos: YouTubeVideo[],
	channels: YouTubeChannel[],
): VideoFeedIndex {
	const mutedChannelIds = new Set(
		channels.filter((channel) => channel.isMuted).map((channel) => channel.id),
	);
	const items = videos
		.map((video) => {
...
function deduplicateByTitle(items: IndexedVideo[]) {
	const seen = new Set<string>();
	return items.filter((item) => {
		if (!item.normalizedTitle) return true;
		if (seen.has(item.normalizedTitle)) return false;
		seen.add(item.normalizedTitle);
		return true;
	});
}

function boostByKeywords(items: IndexedVideo[], boostedKeywords: string[]) {
	return [
		...items.filter((item) =>
			matchesAnyKeyword(item.keywordText, boostedKeywords),
		),
		...items.filter(
			(item) => !matchesAnyKeyword(item.keywordText, boostedKeywords),
		),
	];
}

export function filterIndexedVideos(
	index: VideoFeedIndex,
	options: VideoFilterOptions,
) {
	...
	const filteredItems = index.items.filter((item) =>
		matchesVideoFilters(item, index, options, normalizedSearch, mutedKeywords),
	);

	const dedupedItems = options.hideDuplicateTitles
		? deduplicateByTitle(filteredItems)
		: filteredItems;

	if (boostedKeywords.length === 0) return dedupedItems;
	return boostByKeywords(dedupedItems, boostedKeywords);
}
