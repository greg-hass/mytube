...
function parseTakeoutCSV(csv: string): OPMLChannel[] {
	...
	const idIndex = headers.findIndex(
		(header) => header === "channelid" || header === "channel",
	);
	const urlIndex = headers.findIndex(
		(header) => header === "channelurl" || header === "url",
	);
	const titleIndex = headers.findIndex(
		(header) =>
			header === "channeltitle" || header === "title" || header === "name",
	);
...
export function parseOPML(opmlXML: string): OPMLChannel[] {
	...
	try {
		...
		throw new Error(
			`Failed to parse OPML XML: ${error instanceof Error ? error.message : String(error)}`,
		);
	}
 ...
 */
export function channelsToSubscriptions(
	channels: OPMLChannel[],
): StoredSubscription[] {
	const now = Date.now();
	...
	return channels.map((channel) => ({
		...
		...(channel.isFavorite ? { isFavorite: true } : {}),
		...(channel.isMuted ? { isMuted: true } : {}),
		...(channel.group ? { group: channel.group } : {}),
	}));
}
 ...
 */
export function parseOPMLToSubscriptions(
	opmlXML: string,
): StoredSubscription[] {
