		...
		(db) =>
			new Promise<T[]>((resolve, reject) => {
				try {
					...
					request.onerror = () => {
						reject(
							new Error(`Cursor operation failed: ${request.error?.message}`),
						);
					};
				} catch (error) {
					reject(error);
...
export async function addCachedVideos(videos: CachedVideo[]): Promise<void> {
	...
	return new Promise((resolve, reject) => {
		...
		transaction.onerror = () => {
			reject(
				new Error(
					`Bulk video cache insert failed: ${transaction.error?.message}`,
				),
			);
		};
	});
...
export async function getAllCachedVideos(): Promise<CachedVideo[]> {
	return executeTransaction(VIDEOS_CACHE_STORE, "readonly", (store) =>
		store.getAll(),
	);
}
 ...
 */
export async function getCachedVideosByChannel(
	channelId: string,
): Promise<CachedVideo[]> {
 ...
 */
export async function removeCachedVideosByChannel(
	channelId: string,
): Promise<void> {
	const db = await getDB();
	const transaction = db.transaction(VIDEOS_CACHE_STORE, "readwrite");
	...
	return new Promise((resolve, reject) => {
		...
		request.onerror = () => {
			reject(
				new Error(
					`Failed to delete videos for channel: ${request.error?.message}`,
				),
			);
		};
	});
...
export async function removeOldCachedVideos(maxAge: number): Promise<number> {
	...
	return new Promise((resolve, reject) => {
		...
		request.onerror = () => {
			reject(
				new Error(`Failed to delete old videos: ${request.error?.message}`),
			);
		};
	});
}
...
export async function clearAllData(): Promise<void> {
	await Promise.all([clearAllSubscriptions(), clearAllCachedVideos()]);
}
