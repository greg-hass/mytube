...
Task:
You are the executor for implementation plan 008 at plans/008-dead-code-cleanup.md in the repo root.

Read the full plan file FIRST, then follow it step by step. Run every verification command and confirm the expected result before moving on. Touch only the files listed as in scope (server/channel-search.js, server/opencode-channel-resolver.js, server/opencode-channel-resolver.test.js, server/llm-channel-resolver.js). If any STOP condition occurs, stop immediately and report.

IMPORTANT: Step 1 is a verification gate — run the grep commands to confirm the dead code is truly dead BEFORE deleting anything. If any grep returns live callers, STOP.
