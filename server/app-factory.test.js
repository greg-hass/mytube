...
function buildAppStore(databaseFile) {
    ...
    return {
        DEFAULT_DATA: { subscriptions: [], settings: {}, watchedVideos: [], redirects: {} },
        DEFAULT_VIDEO_CACHE: { videos: [], lastUpdated: null, totalChannels: 0, totalVideos: 0, channelRefreshes: {} },
...
function buildFeedAggregatorStub(overrides = {}) {
    return {
        getAggregationStatus: () => ({ running: false, queued: false, lastUpdated: null, totalChannels: 0 }),
...
function buildApp({ databaseFile, apiKey = TEST_TOKEN, feedAggregator, config = {} } = {}) {
    ...
    return { app: result.app, appStore, aggregator, databaseFile, thumbnailRateLimiter: result.thumbnailRateLimiter };
}

async function bootstrap(databaseFile) {
    const { app, appStore, aggregator, thumbnailRateLimiter, databaseFile: dbFile } = buildApp({ databaseFile });
    await appStore.init({
        defaultData: appStore.DEFAULT_DATA,
        defaultVideoCache: appStore.DEFAULT_VIDEO_CACHE,
    });
    return { app, appStore, aggregator, thumbnailRateLimiter, databaseFile: dbFile };
}

async function cleanup({ appStore, databaseFile }) {
    appStore.close();
    await fs.promises.rm(path.dirname(databaseFile), { recursive: true, force: true });
}

function authedRequest(app) {
    const builder = {
        get: (path) => request(app).get(path).set('Authorization', `Bearer ${TEST_TOKEN}`),
        post: (path) => request(app).post(path).set('Authorization', `Bearer ${TEST_TOKEN}`),
    };
    return builder;
}
...
describe('createApp integration', () => {
    ...
    it('POST /api/sync without If-Match accepts the write and bumps the revision', async () => {
        const response = await authedRequest(resources.app)
            .post('/api/sync')
            .send({
                subscriptions: [{ id: 'UCaaaaaaaaaaaaaaaaaaaaaa', title: 'One', thumbnail: '', description: '' }],
                settings: {},
                watchedVideos: [],
            });
        expect(response.status).toBe(200);
    ...
    it('POST /api/sync with stale If-Match returns 412 and current ETag', async () => {
        const first = await authedRequest(resources.app)
            .post('/api/sync')
            .send({ subscriptions: [{ id: 'UCaaaaaaaaaaaaaaaaaaaaaa', title: 'One' }], settings: {}, watchedVideos: [] });
        expect(first.status).toBe(200);

        const second = await authedRequest(resources.app)
            .post('/api/sync')
            .send({ subscriptions: [{ id: 'UCbbbbbbbbbbbbbbbbbbbbbb', title: 'Two' }], settings: {}, watchedVideos: [] });
        ...
        const stale = await authedRequest(resources.app)
            .post('/api/sync')
            .set('If-Match', '"0"')
            .send({ subscriptions: [{ id: 'UCcccccccccccccccccccccc', title: 'Three' }], settings: {}, watchedVideos: [] });
        ...
        expect(after.body.subscriptions.map((s) => s.id)).toEqual(['UCbbbbbbbbbbbbbbbbbbbbbb']);
    });
    ...
    it('GET /api/videos/status includes activeChannels from the aggregator', async () => {
        const active = [{ id: 'UCaaaaaaaaaaaaaaaaaaaaaa', title: 'Active', lastSuccessfulFetchAt: null, inFlightSince: null, lastError: null }];
        const { app, appStore, databaseFile } = buildApp({
            databaseFile: createTempDatabaseFile(),
            feedAggregator: buildFeedAggregatorStub({ getActiveChannels: async () => active }),
        });
        await appStore.init({ defaultData: appStore.DEFAULT_DATA, defaultVideoCache: appStore.DEFAULT_VIDEO_CACHE });
        try {
