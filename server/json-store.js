...
async function backupExistingJson(file) {
	...
	const backupFile = path.join(
		backupDir,
		`${parsedPath.name}.${timestamp}.bak.json`,
	);
...
async function pruneBackups(backupDir, basename) {
	...
	const backups = entries
		.filter(
			(entry) =>
				entry.startsWith(`${basename}.`) && entry.endsWith(".bak.json"),
		)
		.sort();
	...
	await Promise.all(
		backups
			.slice(0, excess)
			.map((entry) => fs.rm(path.join(backupDir, entry), { force: true })),
	);
}
...
async function findNewestValidBackup(file) {
	...
	const backups = entries
		.filter(
			(entry) =>
				entry.startsWith(`${parsedPath.name}.`) && entry.endsWith(".bak.json"),
		)
		.sort()
		.reverse();
