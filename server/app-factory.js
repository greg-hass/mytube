...
const APP_PACKAGE = readPackageMetadata('../package.json', { version: 'unknown' });
...
function createApp({
    ...
    const app = express();
    const allowedOrigins = parseAllowedOrigins(config.allowedOrigins ?? process.env.ALLOWED_ORIGINS);
    const apiKey = config.apiKey ?? process.env.SERVER_API_TOKEN ?? '';
    const allowInsecure = config.allowInsecureUnauthenticatedApi
        ?? (process.env.ALLOW_INSECURE_UNAUTHENTICATED_API === 'true');
    const rateLimitWindow = config.rateLimitWindowMs
    ...
    const defaultData = config.defaultData ?? appStore.DEFAULT_DATA ?? { subscriptions: [], settings: {}, watchedVideos: [], redirects: {} };
    const defaultVideoCache = config.defaultVideoCache ?? appStore.DEFAULT_VIDEO_CACHE ?? { videos: [], lastUpdated: null, totalChannels: 0, totalVideos: 0, channelRefreshes: {} };

    if (logStartup) {
        console.log(`[startup] Allowed browser origins: ${describeAllowlist(new Set(allowedOrigins))}`);
    }
    ...
    app.post('/api/sync', asyncHandler(async (req, res) => {
        ...
        const savedData = await appStore.updateData(defaultData, (existingData) => {
    ...
    app.post('/api/videos/refresh', asyncHandler(async (req, res) => {
        ...
        res.json({ success: true, message: 'Refresh started in background. Check back in a few minutes.' });
